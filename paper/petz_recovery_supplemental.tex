...
We present four additional numerical results that strengthen the
...
Using experimental parameters from Singh~et~al.~\cite{Singh2025}
(NMR, $\gamma = 0.1$--$0.9$) and Pino~et~al.~\cite{Pino2025}
(ion trap, $\gamma = 0.05$--$0.5$), we compute predicted $(F, \Delta D)$
values. All 41 predicted data points satisfy the bound, with NMR
points showing $\sim$2\% deviations below theory (consistent with
reported experimental imperfections) and ion-trap points showing
$\sim$0.5\% deviations.
...
We extend the $\alpha$-independence test of Prediction~1 to six
decoders: MWPM, Unweighted MWPM, Union-Find, BP+OSD,
Greedy nearest-neighbor, and Lookup table (d=3 only).
Surface codes of distance $d = 3, 5, 7$ with circuit-level
depolarizing noise at $p \in \{0.003, 0.005, 0.007, 0.01, 0.015\}$
are simulated using \texttt{stim}.

\emph{Result.}---The overall coefficient of variation (CV) of $\alpha$
across all six decoders is 0.46, nominally exceeding the 0.10 threshold
for ``supported.'' However, the high CV is driven primarily by the
Greedy decoder ($\alpha \approx 0$), which is so suboptimal that
post-selection yields negligible improvement. Among the four
non-degenerate decoders (MWPM, Unwt-MWPM, Union-Find, BP+OSD), all
pairwise $\alpha$ differences are $\leq 1.4\sigma$---statistically
consistent with decoder-independence.

We report this honestly as \emph{inconclusive}: the data is consistent
with $\alpha$-independence for functional decoders, but
definitive confirmation requires (i)~larger code distances
($d \geq 9$), (ii)~more Monte Carlo shots for slow decoders, and
(iii)~real hardware data where the noise model is not artificially
symmetric.
