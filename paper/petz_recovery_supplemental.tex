...
(NMR, $\gamma = 0.1$--$0.9$) and Pino~et~al.~\cite{Pino2025}
...
%=======================================================================
\section{S17: Saturation Theorem---Full Proof}
\label{sec:S17}
%=======================================================================

We prove Theorem~4 of the main text: sufficient conditions for
exact saturation of the Junge et~al.\ bound
$F^2(\rho,\Petz_{\sigma,\chan}(\chan(\rho))) \geq \exp(-\Delta D)$.

\subsection{S17.1: Statement}

Let $\chan$ be CPTP with Kraus operators $\{E_k\}$,
$\sigma$ full-rank, and $\rho = \ket{\psi}\!\bra{\psi}$ pure.
If (i)~$\chan(\rho) = \rho$, (ii)~$[\rho,\sigma] = 0$, and
(iii)~$[\rho,\chan(\sigma)] = 0$, then
\begin{equation}
F^2 = \frac{\bra{\psi}\sigma\ket{\psi}}%
  {\bra{\psi}\chan(\sigma)\ket{\psi}}
= \exp(-\Delta D).
\end{equation}

\subsection{S17.2: Proof of sufficiency}

Work in a basis where $\ket{\psi} = \ket{0}$, so
$\rho = P \equiv \ket{0}\!\bra{0}$.

\emph{Step~1: Eigenvalue structure from condition~(i).}
Since $\chan(P) = \sum_k E_k P E_k^\dagger = P$, each Kraus
operator satisfies $E_k\ket{0} = \lambda_k\ket{0}$ for some
$\lambda_k \in \mathbb{C}$ with
$\sum_k |\lambda_k|^2 = 1$.

\emph{Step~2: Block-diagonal structure from conditions~(ii)--(iii).}
Condition~(ii) gives $\sigma = s_0 P \oplus \sigma_\perp$ where
$s_0 = \bra{0}\sigma\ket{0}$.  Condition~(iii) gives
$\chan(\sigma) = \omega_0 P \oplus [\chan(\sigma)]_\perp$ where
$\omega_0 = \bra{0}\chan(\sigma)\ket{0}$.

\emph{Step~3: Relative entropy drop.}
\begin{align}
D(P\|\sigma) &= -\log s_0, \notag\\
D(P\|\chan(\sigma)) &= -\log \omega_0, \notag\\
\Delta D &= \log(\omega_0/s_0).
\end{align}
Therefore $\exp(-\Delta D) = s_0/\omega_0$.

\emph{Step~4: Petz map computation.}
\begin{align}
\chan(\sigma)^{-1/2}\,P\,\chan(\sigma)^{-1/2}
&= \omega_0^{-1}\,P, \notag\\
\chan^\dagger(\omega_0^{-1}P)
&= \omega_0^{-1}\sum_k E_k^\dagger P\, E_k, \notag\\
\bra{0}\chan^\dagger(\omega_0^{-1}P)\ket{0}
&= \omega_0^{-1}\sum_k|\lambda_k|^2 = \omega_0^{-1}.
\end{align}
The full Petz map gives:
\begin{equation}
\bra{0}\Petz(P)\ket{0}
= s_0^{1/2}\cdot\omega_0^{-1}\cdot s_0^{1/2}
= s_0/\omega_0.
\end{equation}
Since $\rho = P$ is pure,
$F^2 = \bra{0}\Petz(\chan(P))\ket{0}
= \bra{0}\Petz(P)\ket{0} = s_0/\omega_0
= \exp(-\Delta D)$.\hfill$\square$

\subsection{S17.3: Condition~(i) is not necessary}

\emph{Counterexample.}---The completely depolarizing channel
$\chan(\rho) = I/d$ satisfies $\chan(P) = I/d \neq P$ for any
pure $P$, violating condition~(i).  Yet with $\sigma = I/d$:
\begin{align}
F^2 &= \bra{0}(I/d)\ket{0} = 1/d, \notag\\
\Delta D &= D(P\|I/d) - D(I/d\|I/d) = \ln d - 0 = \ln d,
  \notag\\
\exp(-\Delta D) &= 1/d = F^2.
\end{align}
Saturation holds for every pure input.  This demonstrates that
condition~(i) is sufficient but not necessary.

\subsection{S17.4: Numerical verification}

The saturation theorem is verified numerically across six
channel families in the script
\texttt{numerical/saturation\_verification.py}:
amplitude damping ($\gamma \in \{0.1,\ldots,0.9\}$, ground
state), pure loss bosonic ($d=3$, vacuum), complement of
amplitude damping (anti-degradable), and three-level decay
channels.  All cases satisfying conditions~(i)--(iii) achieve
$F^2/\exp(-\Delta D) = 1$ to machine precision ($<10^{-12}$).
Cases violating any condition show $F^2/\exp(-\Delta D) > 1$.
Total: 46/46 tests pass with zero bound violations.
