...
across all six decoders is 0.46, which does not meet a stringent
threshold for decoder-independence. However, the high CV is driven
almost entirely by the Greedy nearest-neighbor decoder
($\alpha \approx 0$), which is so suboptimal that post-selection
yields negligible improvement---it effectively operates in a
regime where the factorization ansatz does not apply.
Among the four functionally distinct decoders (MWPM, Unwt-MWPM,
Union-Find, BP+OSD), the picture is more encouraging: all pairwise
$\alpha$ differences are $\leq 1.4\sigma$, statistically consistent
with decoder-independence. The Lookup table decoder (available only
at $d = 3$) shows intermediate behavior.

We characterize this as \emph{partial support}: the trend among
functional decoders is positive and consistent with the prediction,
but the evidence is not yet definitive. Stronger confirmation
requires (i)~larger code distances ($d \geq 9$) where the
asymptotic scaling is better resolved, (ii)~higher Monte Carlo
statistics for slow decoders, and (iii)~real hardware data where
the noise model is not artificially symmetric. We encourage
experimental groups with existing surface code data (e.g., from
Refs.~\cite{Acharya2024,Bausch2024}) to extract $\alpha$ for
multiple decoders as a direct test.
