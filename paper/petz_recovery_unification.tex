...
We present two new results.  First, we identify sufficient
conditions under which the recovery bound is exactly saturated
($F^2 = \exp(-\Delta D)$): the saturation value is a Bayesian
posterior ratio, connecting quantum recovery to classical
retrodiction.  Second, we prove that $\sqrt{\tauasym}$ satisfies
a triangle inequality under channel composition---a property
unique to the Petz map that fails for the universal
(rotated Petz) recovery map of Junge et~al.
\end{abstract}
...
We present two new mathematical results.  First, we identify
sufficient conditions under which the Junge et~al.\ bound
$F^2 \geq \exp(-\Delta D)$ is exactly saturated, with a
saturation value given by a Bayesian likelihood ratio
...
We identify sufficient conditions under which the Junge et~al.\
bound $F^2 \geq \exp(-\Delta D)$ is exactly saturated.

\begin{theorem}[Saturation of the Recovery Bound]
\label{thm:saturation}
Let\/ $\chan$ be a CPTP map, $\sigma$ a full-rank reference state,
and $\rho = \ket{\psi}\!\bra{\psi}$ a pure state.  If the
following conditions hold:
\begin{enumerate}
\item[\emph{(i)}] $\chan(\rho) = \rho$ \emph{(fixed point);}
\item[\emph{(ii)}] $[\rho, \sigma] = 0$
  \emph{(input commutes with reference);}
\item[\emph{(iii)}] $[\rho, \chan(\sigma)] = 0$
  \emph{(input commutes with channel output of reference);}
\end{enumerate}
then the Junge et~al.\ bound is exactly saturated:
\begin{equation}
F^2\!\big(\rho,\,\Petz_{\sigma,\chan}(\chan(\rho))\big)
= \exp(-\Delta D)
= \frac{\bra{\psi}\sigma\ket{\psi}}%
       {\bra{\psi}\chan(\sigma)\ket{\psi}}.
\label{eq:saturation}
\end{equation}
\end{theorem}

\emph{Proof sketch.}---Work in a basis where
$\ket{\psi} = \ket{0}$.  Condition~(ii) gives
$\sigma = s_0\ket{0}\!\bra{0} \oplus \sigma_\perp$;
condition~(i) forces $E_k\ket{0} = \lambda_k\ket{0}$ with
$\sum_k|\lambda_k|^2 = 1$.  Then
$\Delta D = \ln(\omega_0/s_0)$ where
$\omega_0 = \bra{0}\chan(\sigma)\ket{0}$, and direct computation
of the Petz map gives
$F^2 = s_0/\omega_0 = \exp(-\Delta D)$.
See~\cite{SM}, Sec.~S16--S17, for the full derivation and
numerical verification across six channel
families.\hfill$\square$

\emph{Bayesian interpretation.}---The saturation value
$F^2 = s_0/\omega_0$ is the ratio of prior to posterior
probability: $P(\psi|\text{prior})/P(\psi|\text{posterior})$.
Saturation occurs precisely when quantum recovery reduces to
classical Bayesian retrodiction on the one-dimensional subspace
spanned by $\ket{\psi}$.

\emph{Examples.}---(a)~Amplitude damping
$\chan_\gamma^{\mathrm{AD}}$ with $\rho = \ket{0}\!\bra{0}$
(ground state) and $\sigma = I/2$:
$F^2 = 1/(1+\gamma) = \exp(-\ln(1+\gamma))$, saturated for
all $\gamma \in [0,1]$.
(b)~Pure loss bosonic channel (transmissivity $\eta$) with
vacuum input: saturated for all $\eta$.
(c)~The complement of the amplitude damping channel (which is
\emph{anti-degradable}) also saturates---degradability is
irrelevant.
(d)~Unital channels (dephasing, depolarizing) are \emph{strictly
non-saturated} in the interior~\cite{SM}.

\emph{Remark.}---Conditions~(i)--(iii) are sufficient but not
necessary.  The completely depolarizing channel
$\chan(\rho) = I/d$ saturates for all pure inputs
($F^2 = 1/d = \exp(-\ln d)$) despite condition~(i) failing.
A complete characterization of necessity remains open.

\emph{Experimental relevance.}---Amplitude damping is the
dominant noise in superconducting qubits ($T_1$ decay) and ion
traps.  Petz recovery has been implemented on both
...
(2)~sufficient conditions for exact saturation of the recovery
bound, with a Bayesian interpretation
...
\emph{Limitations.}---The sufficient conditions in
Theorem~\ref{thm:saturation} are not necessary: the completely
depolarizing channel saturates without satisfying condition~(i).
A full characterization of necessity remains open.
...
\emph{Open questions.}---(i)~What are the necessary and
sufficient conditions for saturation of the Junge et~al.\ bound?
...
the explicit quantum eraser calculation, the saturation theorem
proof (Sec.~S17), the composition sub-additivity proof
(Sec.~S18), post-selection analysis,
