...
These connections are well established in the literature;
...
$\bra{\phi}$. We suggest that Theorem~\ref{thm:unique} provides a canonical
...
consequence of this uniqueness rather than a coincidence of
case-specific linear algebra.
...
with $\Delta D$.  The ordering $I(A;E|B) \leq \Sigma \leq \Delta D$ is
non-trivial only for finite-size baths; see the Supplemental
Material~\cite{SM} for discussion.}; and
...
hence $\tauasym = 0$. In our framework, this is a direct consequence of the equivalence
...
first or the idler first yields identical statistics. In the $\tauasym$ framework, the standard
conclusion~\cite{Zurek2003} that delayed choice requires no
retrocausal explanation follows directly: the closed system has
$\tauasym = 0$, so the temporal order of measurements is irrelevant. The ``need to wait'' for the idler measurement arises
...
contributions are: (1)~the definition of $\tauasym = 1 - F$
as a continuous measure of the arrow of time;
