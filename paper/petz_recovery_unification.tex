...
recovery strategy. Numerical simulations provide partial support:
among four functionally distinct decoders (MWPM, Unweighted MWPM,
Union-Find, BP+OSD), all pairwise $\alpha$ differences are
$\leq 1.4\sigma$, consistent with decoder-independence; however,
a highly suboptimal greedy decoder deviates significantly
(see~\cite{SM}, Sec.~S15.3 for full details). Definitive
confirmation requires larger code distances ($d \geq 9$) and
real hardware data. The factorization structure is independently
...
decoder-independent $\alpha$ receives partial numerical support
(see~\cite{SM}, Sec.~S15.3) and is directly testable on current
hardware~\cite{Acharya2024}; definitive confirmation requires
