...
object governs all four phenomena. (A concurrent and independent
development by Buscemi~et~al.~\cite{Buscemi2024} constructs a
closely related mathematical structure, providing further evidence
for the naturalness of this unification.)
...
The framework generates two testable predictions that go beyond the
mathematical structure established above. While the individual
inequalities in Eq.~\eqref{eq:master} are known results, the
following predictions are \emph{new physical consequences} of
identifying the Petz map as the unique retrodiction functor
...
(code-distance-independent). \emph{What is new:} that the per-distance
scaling $\alpha(p)$ is \emph{independent of the decoder} is a
consequence of our framework---$\Delta D(s)$ depends only on the
noise channel $\chan$ and the code state $\sigma_\code$, not on the
recovery strategy. Numerical simulations support this prediction:
extending to $d \in \{3, 5, 7, 9, 11\}$ with three decoders
(MWPM, Unweighted MWPM, Union-Find), the coefficient of variation
of $\alpha$ across decoders is $\text{CV} = 0.091$ (mean over noise
rates), well below the $0.15$ threshold, with all pairwise
differences $\leq 0.62\sigma$ (see~\cite{SM}, Sec.~S15.3). The factorization structure is independently
supported by Boltzmann-factor failure rates via the Ising--QEC
...
\emph{What is new:} if the Petz map were merely one good recovery map
among many, $\delta_{\mathcal{D}}$ would have no reason to correlate
with decoder performance. But the uniqueness theorem
(Theorem~\ref{thm:unique}) implies that \emph{all} good decoders must
approximate the Petz map, so $\delta_{\mathcal{D}}$ must rank-order
decoder performance. The known decoder
hierarchy~\cite{deMartiOlius2024}---ML $>$ neural
network~\cite{Bausch2024} $>$ MWPM $>$ Union-Find---should therefore
be a hierarchy of retrodiction fidelity. See~\cite{SM}, Secs.~S10
and~S12 for the derivation and numerical verification.
...
$\ln R(\varepsilon, d) = \alpha d + \beta(\varepsilon)$ with
decoder-independent $\alpha$ is numerically supported at
$d = 3$--$11$ with $\text{CV} = 0.091$
(see~\cite{SM}, Sec.~S15.3) and is directly testable on current
hardware~\cite{Acharya2024}; further confirmation with
...
We note that the bound $\tauasym \leq 1 - \exp(-\Sigma/2)$ has not
yet been tested by simultaneous measurement of $F$ and $\Sigma$ on
the same quantum channel; existing evidence comes from theoretical
calculations using experimentally reported channel parameters
(see~\cite{SM}, Sec.~S15.1). Detailed experimental proposals,
including a protocol for simultaneous $F$ and $\Sigma$ measurement on
a superconducting qubit platform and strategies for reanalyzing
existing data, are given in the Supplemental Material~\cite{SM}.
...
the decoder hierarchy, numerical verification of Predictions~1
and~2, the equivalence chain proof, and caveats.
