...
$\Sgrav = -\ln(-g_{00})$.
The identity $\sqrt{-g_{00}} = e^{-\Sgrav/2}$
yields the exponential metric $g_{00} = -e^{-\rs/r}$,
which has no event horizon ($\tauasym < 1$ everywhere)
and agrees with Schwarzschild through second
...
Petz recovery for this channel type has been experimentally
demonstrated~\cite{ChenSongScarani2025}.

No route constructs a canonical gravitational CPTP map with
explicit Kraus operators~\cite{KasprzakTjoa2024}.
We bypass this: any consistent classical-gravity--quantum-matter
coupling is irreversible~\cite{GalleyGiacominiSelby2023},
guaranteeing the existence of an irreversible channel; route~(a)
fixes the coefficient; route~(c) provides a concrete existence
proof.
The modular channel of Trejo-Calder\'{o}n~\cite{TrejoCalderon2025}
is the most promising candidate for a first-principles construction.
...
An alternative route to this identification uses quantum
hypothesis testing~\cite{HayashiYamasaki2025}:
$\Sgrav = D(\rho(r_1)\|\rho(r_2))$, where $\rho(r)$
is the quantum state of a probe at radius $r$.
This definition bypasses the need for an explicit CPTP
map and gives $\Sgrav = -\ln(-g_{00})$ directly from the
Kubo--Martin--Schwinger (KMS) state structure in curved
spacetime.
...
\emph{The exponential metric.}---The identity
$\sqrt{-g_{00}} = e^{-\Sgrav/2}$, taken as
exact, determines the metric
uniquely~\cite{Papapetrou1954,Yilmaz1958}:
\begin{equation}
...
the exponential metric is the geometry whose redshift factor
\emph{equals} the Petz recovery fidelity bound, $\sqrt{-g_{00}} = e^{-\Sigma/2}$,
taken as an exact identity rather than a one-sided inequality.
The spatial component $g_{rr} = e^{+\rs/r}$ follows from the
isometry condition $g_{00}\cdot g_{rr} = -1$ (Dicke's scalar
refractive index~\cite{Dicke1957}), which is an independent
assumption---information theory constrains only $g_{00}$.
Predictions are thus \emph{robust} (no-horizon theorem,
$\nref = 1/\Fbound$, decoherence bound---depending only on
$g_{00}$) or \emph{conditional} (shadow, QNM, echoes---depending
on the full metric).

\emph{Caveat on saturation.}---The JRSWW
bound~\cite{JRSWW2018} is structurally \emph{strict} for
$\Sigma > 0$: the proof proceeds via the Golden--Thompson
inequality, which is an equality only when the relevant
operators commute---precisely the $\Sigma = 0$
case~\cite{Petz1988}.
No known quantum channel saturates the bound for
$\Sigma > 0$.
In weak fields, the gap is $O((\rs/r)^2)$, far below
current measurability.
The exponential metric~\eqref{eq:exp_metric} should
therefore be understood not as the unique consequence of
saturation, but as the geometry defined by the
\emph{identity} $\sqrt{-g_{00}} = e^{-\Sgrav/2}$, which
holds independently of whether the Petz bound is tight.
Whether nature selects this geometry (exponential metric)
or admits a gap (a metric between exponential and
...
($\Sigma > 0$), providing model-independent support for the
premise that gravitational processes produce entropy.
Bai, Buscemi, and Scarani~\cite{BaiBuscemiScarani2024}
independently constructed a fully quantum entropy production
using Bayesian retrodiction---mathematically parallel to the
$\tauasym$ framework but without gravitational application.
De~Vuyst et~al.~\cite{DeVuyst2025} showed via quantum
reference frames that gravitational entropy is
observer-dependent, providing algebraic justification for the
coordinate dependence of $\Sgrav$.
...
Bianconi's gravity-from-entropy framework~\cite{Bianconi2025GfE}
yields Schwarzschild with $O(r^{-4})$ corrections in vacuum~\cite{Thattarampilly2026},
\emph{not} the exponential metric; our information-theoretic
derivation via $\Sgrav = -\ln(-g_{00})$ is independent of that program.
It cannot explain the cosmological constant or dark energy;
...
or extend to general dynamical spacetimes without
modification.
A promising extension uses the Kodama
vector~\cite{Kodama1980}---the unique divergence-free
vector in spherically symmetric spacetimes---giving
$\tauasym_K(R) = 1 - \sqrt{1 - H^2 R^2}$ in FRW,
tracking apparent (not event) horizons;
the deeper foundation is modular flow via crossed-product
algebras~\cite{Witten2022,Chandrasekaran2023}.
...
The exponential metric follows from the identity
$\sqrt{-g_{00}} = e^{-\Sgrav/2}$, with testable
strong-field deviations from Schwarzschild.
The no-horizon theorem holds model-independently:
for any finite $\Sgrav$, $\tauasym < 1$ and complete
information loss cannot occur---regardless of whether
the Petz bound is tight or strict.
...
M.~Hayashi and T.~Yamasaki,
Nat.\ Phys.\ (2025).

\bibitem{BaiBuscemiScarani2024}
Y.~B.~Bai, F.~Buscemi, and V.~Scarani,
``Quantum stochastic entropy production from Bayesian retrodiction,''
