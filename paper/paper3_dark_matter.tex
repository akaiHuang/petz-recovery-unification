...
The effective equation-of-state parameter for the Khronon
perturbations is~\cite{Blanchet2024}
\begin{equation}
\tilde{w}_0 = \frac{\mu_{\mathrm{bg}}^2}{k_{\mathrm{phys}}^2},
\label{eq:w_tilde}
\end{equation}
where $k_{\mathrm{phys}}$ is the physical wavenumber of the
perturbation mode.  At recombination ($z \approx 1100$),
$\mu_0 = H_0/c \approx 2.25 \times 10^{-4}\;
\mathrm{Mpc}^{-1}$ and the first acoustic peak corresponds to
$k_{\mathrm{phys}} = k_{\mathrm{co}} (1+z) \sim 0.01 \times 1101
\approx 11\;\mathrm{Mpc}^{-1}$, giving
\begin{equation}
\tilde{w}_0(z \approx 1100) \sim 4 \times 10^{-10}
\ll 1.
\label{eq:w_dust}
\end{equation}
This is effectively pressureless: the Khronon perturbations
behave as dust at CMB scales, precisely the $c_s = 0$ mode
identified as structurally necessary in
Sec.~\ref{sec:cmb}.
