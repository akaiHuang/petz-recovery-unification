...
\subsection{The channel problem}

[\textsc{new synthesis}]  None of the three routes above
constructs a canonical gravitational CPTP map
$\chan_{\mathrm{grav}}$ from first principles---i.e., a map
with explicit Kraus operators $\{K_i\}$ such that
$\Sigma = D(\rho\|\sigma) - D(\chan_{\mathrm{grav}}(\rho)\|
\chan_{\mathrm{grav}}(\sigma)) = \rs/r$.
This remains an open problem~\cite{KasprzakTjoa2024}.
We bypass it through a combined strategy:
(i)~any consistent coupling of classical gravity to quantum
matter is fundamentally irreversible ($\Sigma > 0$),
guaranteeing the \emph{existence} of an irreversible CPTP
map~\cite{GalleyGiacominiSelby2023};
(ii)~the modular flow route~(a) provides the
coefficient $\rs/r$ without requiring an explicit
channel~\cite{DorauMuch2025};
(iii)~the pure-loss bosonic channel~(c) serves as a concrete
\emph{existence proof} that a CPTP map with
$\Sigma = -\ln\eta = \rs/r$ exists when $\eta = -g_{00}$;
(iv)~axiomatic uniqueness: any CPTP map satisfying additivity
under channel composition and monotonicity under coarse-graining
yields $\Sigma = -c\,\ln(-g_{00})$ with $c > 0$; routes~(a)--(c)
fix $c = 1$.
The recent modular channel construction of
Trejo-Calder\'{o}n~\cite{TrejoCalderon2025}---a CPTP map
built from Gibbs-weighted singular values of the modular
operator---is the most promising candidate for a first-principles
$\chan_{\mathrm{grav}}$, though the explicit computation of
$\Delta D$ in Schwarzschild remains to be carried out.
...
Note that the Jacobson~\cite{Jacobson1995} and
...
This tension between derivability and the specific metric
...
the most distinctive prediction---gravitational-wave echoes
at $\Delta t \approx 4.2\,\rs/c$ ($\sim 2.5$~ms for
$60\,M_\odot$)---has not been searched for, as existing echo
analyses use $\sim 200$~ms templates optimized for Planck-wall
models~\cite{Cardoso2016}.
Dedicated short-delay searches using existing LIGO--Virgo O3/O4
data could test this prediction.
The identification~\eqref{eq:triple} is robust in weak fields
and suggestive in strong fields.
All conditional predictions (shadow, ISCO, QNM, echoes) depend
on the isometry assumption $g_{rr} = e^{+\rs/r}$; the robust
predictions (no-horizon theorem, $\nref = 1/\Fbound$,
decoherence bound) depend only on $g_{00}$.
...
\item Derive $g_{ab}$ from $\tauasym$: the relation is
  ...
  The spatial component $g_{rr}$ requires an independent
  assumption (Sec.~\ref{sec:exponential}); programs that
  derive all components (Jacobson, Dorau--Much)
  yield Schwarzschild, not the exponential metric.
\item Construct a canonical gravitational CPTP map: the bypass
  strategy (Sec.~\ref{sec:identification}) establishes
  $\Sgrav = \rs/r$ without an explicit channel, but the
  Kraus-operator representation of $\chan_{\mathrm{grav}}$
  remains open~\cite{KasprzakTjoa2024,TrejoCalderon2025}.
\item Resolve the firewall paradox~\cite{AMPS2013}: while
  ...
  objects.
\item Apply to general dynamical spacetimes without
  modification: the static formulation requires a timelike
  Killing vector (see Discussion for the Kodama extension).
\end{itemize}
...
(i)~\emph{Dynamic spacetimes.}---The most pressing limitation
is the restriction to static metrics.
The Kodama vector~\cite{Kodama1980}---the unique
divergence-free timelike vector in spherically symmetric
spacetimes---provides a natural extension:
$\Fbound^K(A\!\to\!B) = |K|_A/|K|_B$.
In the static limit, $K^a$ reduces to the Killing vector
and $\Fbound^K = \sqrt{-g_{00}}$, recovering all results of
this paper.
In Vaidya collapse (mass accretion $M(v)$),
$\tauasym_K(r,v) = 1 - \sqrt{1 - 2M(v)/r}$, tracking the
\emph{apparent} horizon---a quasi-local, causal surface---rather
than the event horizon.
In FRW cosmology,
$\tauasym_K(R) = 1 - \sqrt{1 - H^2 R^2}$, reaching
$\tauasym = 1$ at the Hubble radius.
Recent work extends the Kodama approach to axisymmetric
spacetimes~\cite{DorauVerch2024}.
The deeper theoretical foundation lies in modular
flow~\cite{Witten2022,Chandrasekaran2023}: the
Tomita--Takesaki modular automorphism defines $\tauasym$ for
any spacetime and any state, without requiring symmetry.
Crossed-product algebras make gravitational entropy
well-defined and observer-dependent~\cite{KudlerFlam2025},
consistent with the coordinate dependence of $\Sgrav$.
Cirafici~\cite{Cirafici2024} has established fluctuation
theorems in gravitational algebras, confirming that the
Crooks--Petz connection of Paper~I extends to de~Sitter
spacetime.

(ii)~\emph{$\tauasym$ at the Planck scale}: when
...
(iii)~\emph{The canonical channel}: the modular channel
construction of Trejo-Calder\'{o}n~\cite{TrejoCalderon2025}
builds a CPTP map from the singular values of the modular
operator.
Computing its QRE loss in Schwarzschild would either confirm
$\Sigma = \rs/r$ from first principles or reveal a correction.
Kasprzak and Tjoa~\cite{KasprzakTjoa2024} provide the most
rigorous framework for quantum information transmission through
curved-spacetime fields; combining their channel with the
modular construction is a concrete next step.
...
(v)~\emph{Non-Markovian gravity}: if the gravitational
...
% === New references from research round 2026-03-10 ===
...
\bibitem{MoreiraCeleri2026}
S.~V.~Moreira and L.~C.~Celeri,
``Decoherence and entropy production due to quantum
fluctuations of spacetime,''
arXiv:2603.02034 (2026).
