...
$\tauasym(t) \leq 1 - \exp(-E_G t/2\hbar)$ providing an upper bound
on the full dynamics where Penrose gives only one time point.  Three recent results sharpen
...
Each result is labeled [\textsc{known}], [\textsc{new synthesis}],
[\textsc{new formula}], or [\textsc{new bound}].  Attribution to Liu et~al., Jean et~al., and
...
Note that $\Sigma$ here is the JRSWW entropy production of the
dephasing channel, $\Sigma = H_{\mathrm{bin}}((1+p)/2)$, not the
linear identification $E_G t/\hbar$ used in
Sec.~\ref{sec:penrose}.  The two coincide at leading order for small
dephasing but differ at higher orders.
...
\emph{$\tauasym$ bound from Penrose--Di\'{o}si entropy
production}~[\textsc{new bound}].---The Di\'{o}si--Penrose model
posits exponential decoherence $|D(t)| = \exp(-E_G t/\hbar)$.
Artini et~al.~\cite{Artini2025} computed the entropy production rate
for the DP model.  Following their identification
$\Sigma_{\mathrm{DP}}(t) = E_G t/\hbar$, the JRSWW bound gives
the \emph{upper bound}
\begin{equation}
\boxed{
\tauasym(t) \;\leq\; 1 - \exp\!\left(-\frac{E_G\, t}{2\hbar}\right).
}
\label{eq:tau_penrose}
\end{equation}
The actual $\tauasym(t)$ depends on the specific CPTP map implementing
the DP dynamics; the bound constrains any such implementation.
At Penrose's timescale $t_P = \hbar/E_G$:
$\tauasym(t_P) \leq 1 - e^{-1/2} \approx 0.39$.
...
$\tauasym(t) \leq 1 - \exp(-\Sigma(t)/2)$ traces the continuous
...
\emph{Alternative mathematical route.}---The recovery bound
$F \geq \exp(-\Sigma/2)$ requires identifying a CPTP map whose
entropy production equals $\Sigma$.  An alternative approach via
quantum hypothesis testing~\cite{HayashiYamasaki2025} defines
$\Sigma = D(\rho(z_1)\|\rho(z_2))$ directly from quantum states at
different spacetime points, bypassing the need for an explicit channel.
This may strengthen the gravitational applications in
