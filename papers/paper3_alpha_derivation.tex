...
$\alpha_{\mathrm{eff}} \approx 0.01$ corresponds to
$\lambda_D \approx 8$, a natural value requiring no fine-tuning.
...
$\Lambda$CDM at the percent level.  Our Boltzmann
code analysis (main text, Table~I) establishes
\begin{equation}
\alpha_{\mathrm{DBI}} < 0.05
\quad (95\%\;\mathrm{CL}),
\label{eq:bound}
\end{equation}
with best-fit values $\alpha_{\mathrm{eff}} \lesssim 0.01$
(Sachs--Wolfe ratio $R_{\mathrm{SW}} = 1.00$, cold
density ratio $r_c = 0.95$).

\subsection{Natural value from $\lambda_D$}

\derived\ The constraint~\eqref{eq:bound} translates
directly to a lower bound on $\lambda_D$ via
Eq.~\eqref{eq:alpha_bare}:
\begin{equation}
\lambda_D > \sqrt{\frac{Q_0}{2\alpha_{\max}}}
= \sqrt{\frac{1.34}{0.10}} \approx 3.7\,.
\label{eq:lambda_bound}
\end{equation}
For the best-fit value $\alpha_{\mathrm{eff}} = 0.01$:
\begin{equation}
\lambda_D = \sqrt{\frac{Q_0}{2\alpha_{\mathrm{eff}}}}
= \sqrt{\frac{1.34}{0.02}} \approx 8.2\,.
...
$\Omega_{\mathrm{DM}} = 0.265$).  The CMB bound
is $\alpha_{\mathrm{DBI}} < 0.05$.}
\begin{ruledtabular}
\begin{tabular}{lccl}
...
2   & 0.168  & 0.168  & excluded \\
3   & 0.074  & 0.074  & excluded \\
4   & 0.042  & 0.042  & marginal \\
5   & 0.027  & 0.027  & consistent \\
7   & 0.014  & 0.014  & consistent \\
\textbf{8}   & \textbf{0.010}  & \textbf{0.010}  & \textbf{best fit} \\
10  & 0.0067 & 0.0067 & consistent \\
...
%=======================================================================
\section{Why $\lambda_D \approx 8$ is not fine-tuned}
...
over $[1, 100]$, the probability of
$\lambda_D \in [5, 15]$ (the observationally consistent
range) is $\ln(15/5)/\ln(100) = 0.24$---a 24\%
probability, indicating no fine-tuning.
\end{enumerate}

\subsection{Comparison of scales}

The best-fit $\lambda_D \approx 8$ satisfies
\begin{equation}
\frac{\lambda_D}{\delta_0} \approx 24,
\qquad
\frac{\lambda_D}{\delta_{\max}} \approx 7.
\label{eq:ratios}
\end{equation}
Both are $O(10)$ ratios---no large hierarchy is
present.  The physical meaning is transparent:
the DBI nonlinearity activates when
$\delta \sim \lambda_D \sim 8$, well beyond
the cosmological range $\delta \lesssim 1$.
The Khronon thus remains deep in the
quadratic regime at all epochs, with DBI
corrections at the $\sim\!1\%$ level.
...
\end{equation}
For $\Sigma_0 = 0.585$ and $\lambda_D = 8$:
$\alpha_{\mathrm{eff}} = e^{0.293}/128 = 1.34/128
= 0.0105$, confirming the best-fit value.
...
The CMB bound $\alpha < 0.05$ then constrains the
...
constraint $\lambda_D \gtrsim 4$ (equivalently
$\alpha_{\mathrm{DBI}} \lesssim 0.05$) is the
price of working without the tensor field.
...
entropy production.  The CMB constraint
$\alpha_{\mathrm{eff}} < 0.05$ requires
$\lambda_D > 3.7$; the best fit
$\alpha_{\mathrm{eff}} \approx 0.01$ gives
$\lambda_D \approx 8$.  This value:
\begin{enumerate}
\item lies in the natural range
$1 < \lambda_D < 100$ (no fine-tuning);
\item ensures the Khronon remains in the
quadratic ghost condensation regime at all
cosmological epochs ($\delta_{\max}/\lambda_D
\sim 0.13$);
\item produces a testable $\sim\!2\%$ suppression
of $P(k)$ at $k \sim k_J$;
...
$\alpha_{\mathrm{eff}} \approx 0.01$ is
\emph{not} fine-tuned.  It emerges from a
single dimensionless parameter $\lambda_D \sim 8$
