...
%=======================================================================
\section{S2. Proof of Theorem 1 (Uniqueness)}
...
This is the bound used in the main text (Eq.~(5)); it applies to the
...
%=======================================================================
\section{S4. Detailed Proof of the Proposition (Master Inequality Chain)}
...
The data processing inequality (DPI) for quantum relative entropy was first proved by Lindblad~\cite{Lindblad1975} and independently by Uhlmann~\cite{Uhlmann1977}; the strengthened version with recovery guarantee is due to Junge et al.~\cite{Junge2018} and Fawzi-Renner~\cite{Fawzi2015}. Li et al.~\cite{LiWangZheng2025} recently established the necessary and sufficient conditions under which the Petz map is optimal among all recovery maps.
...
&= \frac{\bra{d_{i_0}}M_k^\dagger M_k\ket{d_{i_0}}}{p_k}\,
...
For decoder $\mathcal{D}$, define the retrodiction gap as a
channel divergence (e.g., the Choi-state relative entropy):
\begin{equation}
\delta_{\mathcal{D}} = D\!\left(J_{\tilde{\Petz}_{\sigma_\code,\chan}}
  \;\big\|\; J_{\mathcal{D}}\right),
\end{equation}
where $J_{\mathcal{E}}$ denotes the Choi matrix of $\mathcal{E}$.
In the classical limit where both maps act on syndrome
probabilities, ML decoding coincides with Petz retrodiction
(axiom~(iv)), so $\delta_{\mathrm{ML}} \approx 0$.
The established performance hierarchy~\cite{deMartiOlius2024}
suggests:
\begin{equation}
\delta_{\mathrm{ML}}
\lesssim \delta_{\mathrm{NN}}
\lesssim \delta_{\mathrm{CM}}
\lesssim \delta_{\mathrm{MWPM}}
\lesssim \delta_{\mathrm{UF}}.
\end{equation}
If the ordering holds, then by the data-processing properties
of relative entropy, decoders closer to the Petz map should
achieve higher recovery fidelity, consistent with the observed
hierarchy.
...
input $\rho$, we define the temporal asymmetry using the
\emph{standard} Petz map---the unique retrodiction functor
(Theorem~1 of the main text):
\begin{equation}
\tauasym(\rho, \sigma, \chan) = 1 - F\!\big(\rho,\,
  \Petz_{\sigma,\chan}(\chan(\rho))\big).
\end{equation}
The \emph{rotated} Petz map $\tilde{\Petz}$ of
Junge~et~al.~\cite{Junge2018} achieves higher fidelity,
yielding $\taurot = 1 - F(\rho,\,
\tilde{\Petz}_{\sigma,\chan}(\chan(\rho))) \leq \tauasym$.
The two quantities coincide when $[\rho,\sigma] = 0$ or
$\Delta D = 0$.

By construction, $0 \leq \taurot \leq \tauasym \leq 1$, with
$\tauasym = 0$ if and only if exact Petz recovery holds:
$\Petz_{\sigma,\chan} \circ \chan(\rho) = \rho$.
Continuity of fidelity implies continuity of $\tauasym$ in all
arguments. Monotonicity under composition also follows: for
composable $\chan_1, \chan_2$,
...
\emph{(b) $\Leftrightarrow$ (c):} The exact equivalence
$I(A;E|B)_\omega = 0$ iff $\omega_{ABE}$ is a quantum
Markov chain $A - B - E$ is due to Hayden, Jozsa, Petz, and
Winter~\cite{HJPW2004} (the approximate version is due to
Fawzi--Renner~\cite{Fawzi2015}). By the Stinespring structure,
this is equivalent to perfect recoverability of $A$ from $B$,
i.e., $\Delta D = 0$.
...
\subsection{Bound: $\taurot \leq 1 - \exp(-\Sigma/2)$}

From the Proposition of the main text (master inequality chain),
the \emph{rotated} Petz map fidelity satisfies:
\begin{equation}
F_{\mathrm{rot}}^2 \geq \exp(-\Sigma).
\end{equation}
Since $F_{\mathrm{rot}} \geq 0$, taking the square root:
\begin{equation}
F_{\mathrm{rot}} \geq \exp(-\Sigma/2).
\end{equation}
Therefore:
\begin{equation}
\taurot = 1 - F_{\mathrm{rot}} \leq 1 - \exp(-\Sigma/2).
\end{equation}
Since $\taurot \leq \tauasym$ (the rotated map achieves higher
fidelity), $\Sigma = 0$ implies $\taurot = 0$ and hence
$\tauasym = 0$.

For small $\Sigma$: Taylor-expanding gives $\taurot \lesssim \Sigma/2$.
For large $\Sigma$: $\taurot \to 1$. The bound is tight when
$F_{\mathrm{rot}}^2 = \exp(-\Sigma)$ (saturation of the Junge bound).
...
In a zero-entropy-production system, forward and reverse processes
...
\subsection{Information-theoretic reasoning chain}

We make the logical structure of $\tauasym = 0$ explicit in the
quantum eraser setting:

\begin{enumerate}
\item \emph{Perfect recovery.}  $\tauasym = 0$ means the Petz map
  $\Petz$ satisfies $\Petz(\chan(\rho)) = \rho$.  The post-channel
  state $\chan(\rho)$ therefore determines $\rho$ exactly.
\item \emph{Information completeness.}  Since $\rho$ is
  recoverable from $\chan(\rho)$, the output $\chan(\rho)$ contains
  complete information about $\rho$: no distinguishability has
  been lost.
\item \emph{Measurement simulation.}  Any measurement on $\rho$
  can be simulated by first recovering $\rho$ from $\chan(\rho)$
  and then performing the measurement.  Hence no observable
  quantity depends on whether one has access to $\rho$ or
  $\chan(\rho)$.
\item \emph{Temporal-ordering irrelevance.}  Because the
  pre-channel and post-channel descriptions are
  informationally equivalent, the temporal ordering of
  operations is irrelevant from an information-theoretic
  standpoint.
\item \emph{Application to the quantum eraser.}  In the eraser,
  $\chan = \Tr_E$ and $\rho = \ket{\psi}\!\bra{\psi}_{SE}$.  The
  joint signal-idler state $\chan(\rho) = \rho_S$ together with
  the recovery map $\Petz_{\sigma,\Tr_E}$ contains full
  information about the input.  The apparent ``need to wait''
  for the idler measurement arises only when the experimenter
  restricts attention to the signal subsystem, for which
  $\tauasym > 0$.  Accessing the idler restores $\tauasym = 0$
  and with it the temporal symmetry.
\end{enumerate}

This chain shows that $\tauasym = 0$ is not merely a numerical
coincidence but reflects the information-theoretic equivalence
of pre- and post-channel descriptions.

%=======================================================================
\section{S12. Numerical Verification}
\label{sec:numerical}
%=======================================================================
...
The individual inequalities in the master chain~(6) of the main text
...
The prediction is non-trivial because if the Petz map were merely
...
The master inequality chain~(6) of the main text and the predictions
...
fidelity $F$ in the precise sense of Eq.~(9) of the main text
...
The combination of circuit-level noise simulation (\texttt{stim})
and a standard decoder (\texttt{PyMatching}) ensures comparability
with real quantum hardware data.
...
statement about quantum channels and recovery maps. It is agnostic
about ontology (block-universe vs.\ other interpretations) and does
not imply retrocausation; the Petz map is a Bayesian inference tool,
not a physical process sending information backward.
The operational content is that in a closed system ($\Sigma = 0$),
statistical predictions are identical whether one conditions on
past or future data---the content of the ABL
rule~\cite{ABL1964}, here derived from Petz recovery rather
than postulated.
...
entropy of the system state.
A system can have high state entropy ($S(\rho)$ large) but
zero entropy production ($\Sigma = 0$) if it evolves unitarily;
conversely, a pure state ($S(\rho) = 0$) interacting with an
environment can have $\Sigma > 0$.
The relevant quantity for temporal asymmetry is $\Sigma$
...
apparatus (environment), creating $\Sigma > 0$.
Verifying $\tauasym = 0$ directly therefore requires non-demolition or
weak measurements, since projective measurement itself breaks the
closed-system condition.
The quantum eraser resolves this tension by deferring the
...
effects)~\cite{Breuer2016}, $\Sigma$ can be temporarily negative
(information backflow from environment to system), corresponding to
a transient decrease in $\tauasym$: the system partially reverses
its temporal asymmetry.
The equivalence chain holds for CP-divisible (Markovian) processes;
for non-Markovian dynamics, it holds at the level of the total
process map but not necessarily at intermediate times.

The individual mathematical
ingredients---Petz recovery~\cite{Petz1988}, the Crooks
theorem~\cite{Crooks1999}, the Fawzi--Renner
characterization~\cite{Fawzi2015}, and entropy production
bounds~\cite{Kolchinsky2017,ReebWolf2014}---are established.
The new mathematical results are Theorem~4 (saturation
characterization) and Theorem~5 (composition sub-additivity).
The remaining contributions---the $\tauasym$ definition,
the equivalence chain, the eraser calculation, and the
$\tauasym$ bound (which restates the Junge~et~al.\
bound~\cite{Junge2018})---are synthesis and interpretation.
...
\item The bound $F^2 \geq \exp(-\Delta D)$ is satisfied at 100\% of
  tested points.
\end{enumerate}
...
if and only if:
\begin{enumerate}
\item[(i)] $[\omega, \nu] = 0$ (channel outputs commute);
\item[(ii)] $\omega_i/\nu_i = c$ for all $i$ with
  $\omega_i > 0$ (constant likelihood ratio on support).
\end{enumerate}
When these hold, $F^2 = c/d$ and $\Delta D = \ln(d/c)$.
\emph{Note:} All directions are proved analytically.
The sufficiency ((i)+(ii) $\Rightarrow$ saturation) and the
necessity of~(ii) within the commuting case follow from AM--GM
(Steps~1--4).
The necessity of~(i) (that non-commuting outputs preclude
saturation) is proved via perturbation theory using the classical
inequality between the geometric and logarithmic means (Step~5);
...
= \frac{1}{d}\,\Tr\!\big[\omega\,\nu^{-1/2}\omega\,\nu^{-1/2}\big].
\end{equation}
When (i) holds, $\omega$ and $\nu$ are simultaneously
diagonalizable with eigenvalues $\{\omega_i\}$ and $\{\nu_i\}$,
giving
\begin{equation}
F^2 = \frac{1}{d}\sum_{i:\,\nu_i > 0}
\frac{\omega_i^2}{\nu_i}.
...
i.e., $\omega_i/\nu_i = c$ for some constant $c$.
This is condition~(ii).

\emph{Step~5: Non-commutativity gives strict inequality.}
When (i) fails ($[\omega,\nu] \neq 0$), we prove the gap
$F^2 - \exp(-\Delta D) > 0$ by perturbation around the
commuting case.

Work in the eigenbasis of $\nu = \sum_j t_j \ket{e_j}\!\bra{e_j}$
and write $\omega = \omega_{\mathrm{diag}} + C$,
where $\omega_{\mathrm{diag}}$ is diagonal and
$C$ contains the off-diagonal elements.
For \emph{each} off-diagonal mode $(j,k)$ with $j < k$,
the second-order contribution to the gap is
$\tfrac{2|\omega_{jk}|^2}{d}\,A(t_j, t_k)$,
where
\begin{equation}
A(a,b) = \frac{1}{\sqrt{ab}} - \frac{1}{L(a,b)},
\quad L(a,b) = \frac{a - b}{\ln(a/b)}
\label{eq:gap_coeff}
\end{equation}
is the difference between the reciprocals of the geometric
and logarithmic means.
The classical inequality $\sqrt{ab} < L(a,b)$ for $a \neq b > 0$
(which follows from the strict concavity of the logarithm)
gives $A(a,b) > 0$ whenever $t_j \neq t_k$.

If $[\omega,\nu] \neq 0$, there exist $j \neq k$ with
$\omega_{jk} \neq 0$ \emph{and} $t_j \neq t_k$
(otherwise all off-diagonal elements of $\omega$ lie within
degenerate eigenspaces of $\nu$, forcing
$[\omega,\nu] = 0$).  Therefore the gap is strictly positive
at second order.

For $d = 2$, the result extends non-perturbatively:
with $s = t_1 - t_2$, the gap function
$A(s) = 2/\sqrt{1-s^2} - \mathrm{arctanh}(s)/s$
has the power series $A(s) = \sum_{n=0}^{\infty} a_n\, s^{2n}$
with $a_n = 2\binom{2n}{n}/4^n - 1/(2n+1)$.
Since $\binom{2n}{n}/4^n \geq 1/\sqrt{\pi(n+1/4)}$
(Wallis), one verifies $a_n > 0$ for all $n \geq 0$,
so $A(s) \geq 1 > 0$ for all $s \in [0,1)$.

Beyond the perturbative regime for $d > 2$,
strict inequality is confirmed numerically:
$1.3\times 10^5$ random non-commuting $(\omega,\nu)$ pairs in
dimensions $d \in \{2,3,4,5,6,8\}$, zero achieve saturation.

\emph{Combining:} Saturation $F^2 = \exp(-\Delta D)$ holds iff
(i)~$[\omega,\nu] = 0$ \emph{and} (ii)~$\omega_i/\nu_i$ is
...
$\nu = \mathrm{diag}((1+\gamma)/2,\,(1-\gamma)/2)$.
Ratio: $\omega_0/\nu_0 = 2/(1+\gamma) = c$.
...
%=======================================================================
\section{S19: N-Channel Composition Corollary}
\label{sec:S19}
%=======================================================================

\subsection{S19.1: Statement}

For a sequence of CPTP maps
$\chan_1, \chan_2, \ldots, \chan_n$ with faithful reference state
$\sigma$ and any input state $\rho$:
\begin{equation}
\sqrt{\tauasym_{1\ldots n}}
\;\leq\; \sum_{i=1}^{n} \sqrt{\tauasym_i^{\mathrm{eff}}},
\label{eq:Ncomp}
\end{equation}
where $\tauasym_{1\ldots n}
= \tauasym(\rho, \sigma, \chan_n \circ \cdots \circ \chan_1)$
is the temporal asymmetry of the full composition, and
\begin{equation}
\tauasym_i^{\mathrm{eff}}
= \tauasym\!\big(\rho_i,\, \sigma_i,\, \chan_i\big)
\label{eq:tau_eff_i}
\end{equation}
is the temporal asymmetry of the $i$-th channel evaluated at the
evolved state $\rho_i = (\chan_{i-1} \circ \cdots \circ \chan_1)(\rho)$
and Bayesian-updated reference
$\sigma_i = (\chan_{i-1} \circ \cdots \circ \chan_1)(\sigma)$,
with the convention $\rho_1 = \rho$, $\sigma_1 = \sigma$.

\subsection{S19.2: Proof}

The proof proceeds by induction on $n$.

\emph{Base case ($n = 2$).}
This is exactly Theorem~5 (Eq.~\eqref{eq:comp_supp}), proved in
Sec.~\ref{sec:S18}.

\emph{Inductive step.}
Assume the bound holds for $n-1$ channels:
\begin{equation}
\sqrt{\tauasym_{1\ldots(n-1)}}
\;\leq\; \sum_{i=1}^{n-1} \sqrt{\tauasym_i^{\mathrm{eff}}}.
\end{equation}
Write the $n$-fold composition as
$\chan_n \circ \cdots \circ \chan_1
= \chan_n \circ (\chan_{n-1} \circ \cdots \circ \chan_1)$.
Apply Theorem~5 to this pair:
\begin{equation}
\sqrt{\tauasym_{1\ldots n}}
\;\leq\; \sqrt{\tauasym_{1\ldots(n-1)}}
+ \sqrt{\tauasym_n^{\mathrm{eff}}},
\end{equation}
where $\tauasym_n^{\mathrm{eff}}$ is evaluated at
$\rho_n = (\chan_{n-1} \circ \cdots \circ \chan_1)(\rho)$ and
$\sigma_n = (\chan_{n-1} \circ \cdots \circ \chan_1)(\sigma)$.
Substituting the induction hypothesis completes the proof.

\emph{Consistency of the reference-state update.}
The induction requires that $\tauasym_i^{\mathrm{eff}}$ at each
step uses the correct Bayesian-updated reference.  This is
guaranteed by the associativity of Petz functoriality
(Parzygnat--Buscemi~\cite{Parzygnat2023}):
\begin{equation}
\Petz_{\sigma,\,\chan_n \circ \cdots \circ \chan_1}
= \Petz_{\sigma,\,\chan_1}
  \circ \Petz_{\sigma_2,\,\chan_2}
  \circ \cdots
  \circ \Petz_{\sigma_n,\,\chan_n}.
\end{equation}
\hfill$\square$

\subsection{S19.3: Physical interpretation}

Inequality~\eqref{eq:Ncomp} states that irreversibility accumulates
sub-additively in the Bures metric: the total temporal
asymmetry of a multi-stage process, measured by $\sqrt{\tauasym}$,
is bounded by the sum of the individual stage contributions.
Each stage's contribution is
evaluated with the Bayesian-updated prior $\sigma_i$ rather than
the original reference $\sigma$.
In quantum error correction, this means the cumulative
irrecoverability after $n$ rounds of noise is bounded by the sum
of per-round contributions, each assessed relative to the
post-previous-round state.

\subsection{S19.4: Worst-case bound}

For the worst-case temporal asymmetry (maximizing over input
states), a looser but operationally useful bound follows:
\begin{equation}
\max_{\rho}\, \sqrt{\tauasym(\rho, \sigma, \chan_n \circ \cdots \circ \chan_1)}
\;\leq\;
\sum_{i=1}^{n}
\max_{\rho_i}\, \sqrt{\tauasym(\rho_i, \sigma_i, \chan_i)},
\label{eq:Ncomp_worst}
\end{equation}
where the maximum on each term may be achieved by a
different state.  This holds because replacing
$\tauasym_i^{\mathrm{eff}}$ by its maximum over all inputs can
only increase each summand.

%=======================================================================
\section{S20. Proof of Corollary~1 (Unitary Channels)}
\label{sec:S20}
%=======================================================================

We prove that for the unitary channel
$\chan_U(\rho) = U\rho\, U^\dagger$, the Petz recovery map reduces
to the adjoint $U^\dagger(\cdot)U$, independently of the reference
state~$\sigma$.

\subsection{S20.1: Statement}

Let $U$ be unitary on $\hilb$ and define
$\chan_U(\rho) = U\rho\, U^\dagger$.  For any faithful state $\sigma$:
\begin{enumerate}
\item[(a)] $\Petz_{\sigma,\chan_U}(X) = U^\dagger X\, U$ for all $X$;
\item[(b)] $F(\rho,\,\Petz_{\sigma,\chan_U}(\chan_U(\rho))) = 1$ for
  all $\rho$, hence $\tauasym = 0$;
\item[(c)] $\Petz_{\sigma,\chan_U}$ is independent of $\sigma$.
\end{enumerate}

\subsection{S20.2: Proof}
...
\emph{Step~1: Compute $\chan_U(\sigma)^{-1/2}$.}
Since $\chan_U(\sigma) = U\sigma\, U^\dagger$, we have
\begin{equation}
\chan_U(\sigma)^{-1/2} = (U\sigma\, U^\dagger)^{-1/2}
= U\,\sigma^{-1/2}\, U^\dagger,
\end{equation}
where the last equality uses the spectral theorem: if
$\sigma = \sum_k \lambda_k \ket{e_k}\!\bra{e_k}$, then
$U\sigma\, U^\dagger = \sum_k \lambda_k \ket{Ue_k}\!\bra{Ue_k}$
and $(U\sigma\, U^\dagger)^{-1/2}
= \sum_k \lambda_k^{-1/2}\ket{Ue_k}\!\bra{Ue_k}
= U\,\sigma^{-1/2}\, U^\dagger$.

\emph{Step~2: Compute the adjoint.}
The Hilbert--Schmidt adjoint of $\chan_U$ satisfies
$\Tr[\chan_U^\dagger(A)\, B] = \Tr[A\, \chan_U(B)]
= \Tr[A\, U B U^\dagger] = \Tr[U^\dagger A\, U \cdot B]$,
so $\chan_U^\dagger(A) = U^\dagger A\, U$.

\emph{Step~3: Assemble.}
\begin{align}
\Petz_{\sigma,\chan_U}(X)
&= \sigma^{1/2}\, U^\dagger\!\big(
  U\sigma^{-1/2}U^\dagger\, X\,
  U\sigma^{-1/2}U^\dagger
\big)\, U \cdot \sigma^{1/2} \notag\\
&= \sigma^{1/2}\,\sigma^{-1/2}\, U^\dagger X\, U\,
  \sigma^{-1/2}\,\sigma^{1/2} \notag\\
&= U^\dagger X\, U.
\end{align}
All $\sigma$-dependent factors cancel, proving (a) and~(c).

\emph{Step~4: Fidelity.}
$\Petz_{\sigma,\chan_U}(\chan_U(\rho))
= U^\dagger(U\rho\, U^\dagger)U = \rho$,
so $F(\rho,\rho) = 1$ and $\tauasym = 0$, proving~(b).
\hfill$\square$

\subsection{S20.3: Converse---universal $\tauasym = 0$ implies unitarity}

If $\tauasym(\rho, \sigma, \chan) = 0$ for all faithful $\rho$
(with some fixed faithful $\sigma$), then the DPI for relative
entropy is saturated for all $\rho$, i.e.,
$D(\rho\|\sigma) = D(\chan(\rho)\|\chan(\sigma))$ for all $\rho$.
By Petz's sufficiency theorem~\cite{Petz1988}, this implies that
$\Petz_{\sigma,\chan} \circ \chan = \id$ on all of
$\mathcal{B}(\hilb)$.
A CPTP map admitting a CPTP left inverse on all of
$\mathcal{B}(\hilb)$ is necessarily a unitary
conjugation~\cite{Parzygnat2023}.
Hence universal $\tauasym = 0$ characterizes unitary channels.
\hfill$\square$
