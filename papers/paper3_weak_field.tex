...
derive the CMB constraint $\alpha_{\mathrm{DBI}} < 0.005$ on the
DBI kinetic completion, show that the Bullet Cluster mass-gas
offset is naturally explained by the $\cs^2 = 0$ collisionless
Khronon condensate, compare its parameter economy with $\Lambda$CDM
and AeST, and identify falsifiable predictions.
...
TKS 2016 CMB & $\checkmark^{\ddag}$ & $\rho_K \propto a^{-3}$ ($\delta \lesssim 1$) \\
...
$^{\ddag}$The Khronon equation of state $w = \delta/(2{+}\delta) \sim O(1)$
during matter domination, but $\rho_K \propto a^{-3}$ is
maintained by the identity $\mu^2\delta = I_0/(2a^3)$.
The mapping from $w$ to GDM $w_{\mathrm{GDM}}$
constrained by TKS~\cite{TKS2016} requires a full Boltzmann
solver analysis, which is planned but not yet performed.
...
%-----------------------------------------------------------------------
\subsection{Discussion of CMB results}
...
CMB fit demonstrated? & Sec.~\ref{sec:cmb}$^*$ & Yes~\cite{SZ2021prl} \\
\end{tabular}
\end{ruledtabular}
\end{table}

$^*$Section~\ref{sec:cmb} demonstrates CMB consistency with the
constraint $\alpha_{\mathrm{DBI}} < 0.005$ (CLASS precision,
$< 0.01\%$).  A full MCMC analysis with the Planck likelihood
...
offset from the baryonic gas~\cite{Clowe2006}.  In the Khronon
framework, this is naturally explained: the ghost-condensation
condition $K'(Q_0) = 0$ ensures $\cs^2 = 0$ at the background level,
meaning the Khronon field behaves as \emph{pressureless dust} during
cluster collisions.  Like CDM particles, the Khronon energy density
passes through the collision region without interacting with the gas,
reproducing the observed mass-gas separation.  The small DBI
correction ($\alpha_{\mathrm{eff}} < 0.005$) introduces negligible
pressure effects at cluster scales ($k \ll k_J$), since the
scale-dependent sound speed~\eqref{eq:cs2_DBI} satisfies
$\cs^2 \approx \alpha_{\mathrm{DBI}}(k/k_J)^2 \ll 1$ for the
...
component, with its collisionless nature guaranteed by the same ghost
condensation mechanism ($\cs^2 = 0$) that ensures CDM-like
perturbation growth in the CMB (Sec.~\ref{sec:cmb}).
...
\emph{Constant $\alpha$ as diagnostic.}---The constant-$\alpha_{\mathrm{DBI}}$
parametrization used in Secs.~\ref{sec:alpha_constraint}
and~\ref{sec:pk_constraints} is a diagnostic tool, not the physical
model.  The ghost condensation mechanism naturally produces
time-dependent $\cs^2(a) \propto a^{-6}$
[Eq.~\eqref{eq:cs2_decay}], which reconciles the CMB signal
with structure formation constraints.  A full analysis using the
Blanchet--Skordis Khronon-Tensor framework~\cite{BS2025} with
time-binned $\cs^2$ is left for future work.
...
\item The \textbf{cosmological sector} $K(Q) = \mu^2(Q-1)^2$
  ...
  the cosmological dark matter density, CDM-like perturbations
  ($\cs^2 = 0$), and passes 10 of 11~low-redshift observational
  tests with one marginal (assuming running $\mu_{\mathrm{bg}} = H/c$).
  The DBI kinetic completion introduces a constrained
  deviation $\alpha_{\mathrm{DBI}} < 0.005$ (CMB bound,
  CLASS precision), naturally suppressed by ghost condensation
  to $\alpha_{\mathrm{eff}} \lesssim 0.005$ for $\lambda_D > 10$.
  The Bullet Cluster mass-gas offset is naturally
  explained by the collisionless ($\cs^2 = 0$) Khronon
  condensate.
...
$\cs^2 = 0$, stability), what is assumed
