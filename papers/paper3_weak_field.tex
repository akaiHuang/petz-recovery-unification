...
derive the CMB constraint $\alpha_{\mathrm{DBI}} < 0.05$ on the
...
constraint $\alpha_{\mathrm{DBI}} < 0.05$.  A full MCMC analysis
with the Planck likelihood is planned but not yet performed.
...
correction ($\alpha_{\mathrm{eff}} \sim 0.01$) introduces negligible
...
\item The \textbf{cosmological sector} $K(Q) = \mu^2(Q-1)^2$
  ...
  deviation $\alpha_{\mathrm{DBI}} < 0.05$ (CMB bound),
  naturally suppressed by ghost condensation to
  $\alpha_{\mathrm{eff}} \approx 0.01$ for $\lambda_D \approx 5$.
